...
jellobelt theme uses [Webpack](https://webpack.js.org) to compile and
bundle SASS and JS.
...
Go to the root of jellobelt theme and run the following commands:
`nvm use`
...
`drush en jellobelt -y`
`drush config-set system.theme default jellobelt -y`
